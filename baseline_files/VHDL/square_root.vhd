...
begin
   ...
   begin
      if rst_n = '0' then
         ...
         -----------------------------------------------------------------------
         -- Stage 0
         -- Build the initial remainder and trial-root values from the top two
         -- input bits, then perform the first subtract operation.
         -----------------------------------------------------------------------
         alu_remainder_in(0)             := (others => '0');
         alu_remainder_in(0)(1 downto 0) := unsigned(radicand_in(data_width-1 downto data_width-2));
         ...
         remainder_pipe(0)(data_width-1 downto data_width-2) <= alu_result(0)(1 downto 0);
         ...
         -- Each stage consumes two more input bits, updates the partial
         -- remainder, and appends one bit to the square-root result.
         -----------------------------------------------------------------------
         for i in 1 to PIPE_STAGES-1 loop
            ...
            alu_remainder_in(i)(1 downto 0) := input_pipe(i-1)(data_width-1 downto data_width-2);

            alu_root_in(i) := (others => '0');
            alu_root_in(i)(ALU_HEAD_WIDTH + i - 2 downto 2) := root_pipe(i-1)(i-1 downto 0);
            ...
            remainder_pipe(i)(data_width-i-1 downto data_width-(2*i)-2) <= alu_result(i)(i+1 downto 0);
